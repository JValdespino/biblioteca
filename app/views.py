...
def consulConsulta_int(request):
    p=models.consultaInt.objects.all()
    return render(request, 'consultaConsultaInt.html')
...
def guardarconsulta(request):
        if 'idCon' in request.POST and 'isbn' in request.POST and 'NombreLe' in request.POST and 'Ap' in request.POST and 'Am' in request.POST and 'Edad' in request.POST and 'Tel' in request.POST and 'Dir' in request.POST and 'EscP' in request.POST:
            idCon=request.POST['idCon']
            isbn=request.POST['isbn']
            ...
            EscP=request.POST['EscP'] 
            p=models.consultaInt(idCon=idCon, isbn=isbn, NombreLe=NombreLe, Ap=Ap, Am=Am, Edad=Edad, Tel=Tel, Dir=Dir, EscP=EscP)
            p.save()
            return redirect('consultaInt', {'msg':'Consulta guardada'})

        else:
            return render(request, 'consultaInt.html', {'msg':'No se puede registrar'})

def elimiConsul(request):
    if 'idCon' in request.POST:
        p=models.consultaInt.objects.get(idCon=request.POST['idCon'])
        p.delete()
    return redirect('consultaConsultaInt')

def ediConsul(request):
    if 'idCon' in request.POST:
        ...
        p=models.consultaInt(idCon=idCon, isbn=isbn, NombreLe=NombreLe, Ap=Ap, Am=Am, Edad=Edad, Tel=Tel, Dir=Dir, EscP=EscP)
        p.save()
        return redirect('consultaConsultaInt', {'msg':'Modificacion realizada'})
